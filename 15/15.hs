...
ingredient s = case words s of
  ...
  where parse = read . init

cookies :: [Ingredient] -> ([Ingredient], [Cookie])
cookies xs = (xs, yss)
--  where yss = concatMap permutations $ [[x, y] | x <- [1..100], y <- [1..100], x + y == 100]
    where yss = concatMap permutations $ [[w, x, y, z] | w <- [1..100], x <- [1..100], y <- [1..100], z <- [1..100], w + x + y + z == 100]
...
score xs ys = sc capacity * sc durability * sc flavour * sc texture
  where sc p = norm $ sum $ zipWith (*) (map p xs) ys
