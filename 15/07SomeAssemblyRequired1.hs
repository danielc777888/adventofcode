...
solve :: String -> String
solve = showWires . sort . run . sort . map instruction . lines

run :: [Instruction] -> [Wire]
run xs = if all resolved xs then wires else run (updateInstructions xs wires)
  where wires = filter (\x -> snd x /= blank) (map wire xs)
...
runInstruction (WireInstruction (x, y) (x', y')) = WireInstruction (x, y) (x', y)
...
updateInstruction' i (x:xs) = if resolved ui then ui else updateInstruction' ui xs
  where ui = updateInstruction i x
...
data Instruction = SignalInstruction Wire 
                 | GateInstruction Gate Wire
                 | WireInstruction Wire Wire deriving (Eq, Ord, Show)
