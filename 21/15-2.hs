-- base on textbook, algorithm design with haskell, ch 9 greedy algorithms with graphs
...
solve :: String -> String
-- solve = show . length . snd . fst . mkGraph .  fullMap 5 . lines
solve = show . risk . dijkstra . mkGraph . fullMap 5 . lines

risk :: Path -> Int
risk (vs, d) = d

fullMap :: Int -> [String] -> [String]
fullMap n xs = concatMap (\n -> map concat (transpose (xs'!!n))) [0..(n - 1)]
    where xs' = map (expandMap n) $ expandMap n xs

expandMap :: Int -> [String] -> [[String]]
expandMap 0 xs = []
expandMap n xs = xs:expandMap (n-1) xs'
    where xs' = map(\x -> map incRisk x) xs

incRisk :: Char -> Char
incRisk c = intToDigit (if c' > 9 then 1 else c')
    where c' = digitToInt c + 1
...
mkAdjArray vl cl vs arr = listArray (1, vl) es
    where es = map(\v -> let r = if v `mod` cl == 0 then [] else [(rv, arr!rv)]
