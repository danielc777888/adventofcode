...
mkGame (x : xs) = (ns, bs)
  where
    ns = map read $ words (map (\c -> if c == ',' then ' ' else c) x)
    bs = mkBoards xs
...
mkBoards xs = b : mkBoards (dropWhile (\x -> not (null x)) (drop 1 xs))
  where
    b = mkBoard $ takeWhile (\x -> not (null x)) (drop 1 xs)

mkBoard :: [String] -> Board
mkBoard xs = (rows, cols)
  where
    ns = map (\x -> map (\y -> (read y) :: Int) (words x)) xs
...
play ((x : xs), bs) = if null nws then (x, head ws) else play (xs, nws)
  where
    bs' = map (\(rs, cs) -> (map (\r -> S.delete x r) rs, map (\c -> S.delete x c) cs)) bs
    ws = filter wins bs'
