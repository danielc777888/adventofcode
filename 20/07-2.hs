...
expand :: Bag -> [Bag] -> [Bag]
expand b bs' = [b] ++ if null bs then [] else concat $ map (\x ->
                                                              let f = find x bs'
                                                              in if isJust f then expand (fromJust f) bs' else []) bs
  where bs = bags' b

counts :: Bag -> Int -> [Bag] -> Int
counts _ _ [] = 1
counts b n ys = n * sum (map (\c -> let d = fromJust (find c ys)
                                        in num c + counts d (num c) ys) bs)
                where bs = bags' b
