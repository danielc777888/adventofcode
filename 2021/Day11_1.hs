...
solve :: String -> String
solve = show . snd . simulate 100 0 . mkGrid . lines
...
simulate 0 nf m = (m, nf)
simulate n nf m = simulate (n-1) (nf + nf') m'
    where (m', nf') = step (M.map (+1) m) S.empty

step :: Grid -> Flashes -> (Grid, Int)
step m s
  | S.size s == n' = (m', n')
  | otherwise = step m' s'
  where  (m', s') = M.foldrWithKey (\k v (x, y) -> if v > 9 && not (S.member k y) then flash k x y else (x, y)) (m, s) m
         n' = S.size s'

flash :: (Int, Int) -> Grid -> Flashes -> (Grid, Flashes)
flash k m s = (m'', s')
    where mk = M.findMax m
...
adjacent (x, y) (nx, ny) = br
    where t = if y-1 < 0 then [] else [(x,y-1)]
